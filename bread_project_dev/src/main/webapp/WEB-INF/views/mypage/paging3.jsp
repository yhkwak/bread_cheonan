...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
		...
		<c:if test="${pageNav.pageNum > 10}">
			<a href="likeList.do?pageNum=1&pageBlock=1">&lt;&lt;</a>&nbsp;
    		<a href="likeList.do?pageNum=${(pageNav.pageBlock-2)*pageNav.pages_pageBlock+1}&pageBlock=${pageNav.pageBlock-1}">이전</a>
		</c:if>
		...
		<c:forEach var="p" begin="${(pageNav.pageBlock-1)*pageNav.pages_pageBlock+1}" end="${pageNav.pageBlock*pageNav.pages_pageBlock}">
			<c:if test="${p le pageNav.total_pageNum}">
				<a class="paging_a" href="likeList.do?pageNum=${p}&pageBlock=${pageNav.pageBlock}"> <!-- 현재 페이지를 빨간색으로 출력 -->
		...
		<c:if test="${pageNav.total_pageNum > (pageNav.pageBlock*pageNav.pages_pageBlock)}">
			<a href="likeList.do?pageNum=${(pageNav.pageBlock*pageNav.pages_pageBlock)+1}&pageBlock=${pageNav.pageBlock+1}">다음</a>&nbsp;
    <a href="likeList.do?pageNum=${pageNav.total_pageNum}&pageBlock=${pageNav.last_pageBlock}">&gt;&gt;</a>
		</c:if>